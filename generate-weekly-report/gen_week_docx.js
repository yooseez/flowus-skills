...
// Merge similar work items: if two items share a common prefix >= 5 chars,
// or one contains the other, keep the shorter one
...
function buildWeeklySummary() {
  ...
  for (const person of sortedPersons) {
    ...
    let summary = `${person}（${personHours[person]}h，参与${personDays[person].size}天）：\n`;
    const parts = [];
    for (const proj of projList) {
      ...
      let brief = `${cleanName(proj)}（${hrs}h，参与${days.size}天）`;
      ...
      // Step 2: If progress notes not meaningful, fall back to task names
  ...
  sections: [
    ...
    // Main content
    {
      ...
      children: [
        ...
        heading("四、本周工作总结"),
        ...weeklySummaries.flatMap(s => s.split("\n").map(line => new Paragraph({
          spacing: { before: 40, after: 40 },
          children: [new TextRun({ text: line, font: FONT, size: 21, bold: line.endsWith("：") })],
        }))),
      ],
    },
  ],
