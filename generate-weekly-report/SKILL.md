...
### 报告结构
1. **封面**（纵向）：标题"项目开发团队周报" + 日期范围 + 统计概览表 + 说明文字"本报告由每日日报自动总结生成"
2. **一、人员投入统计**：表格（显示"岗位 姓名"）
3. **二、项目投入统计**：表格（参与人员列只显示姓名，不显示岗位）
4. **三、人员×项目投入矩阵（工时）**：横向表格（显示"岗位 姓名"，人员列宽14%）
5. **四、项目本周工作总结**：按项目汇总，系列任务显示详细列表
6. **五、人员本周工作总结**：按人员+项目汇总
- ❌ **不含"下周工作计划"**
...
- **系列任务显示规则**：
  - 少于8项：显式列出，格式 `XX工作部署、专项工作、工作要点等{后缀}`（去掉公共前缀）
  - 8项及以上：使用通用名称 `XX相关工作：包括A、B、C等`（不再区分后缀）
  ...
  - 项目工作总结也保留显式列出格式（不再通过 `explicitListingMap` 简化为 `XX相关功能开发`）
